...
function parseField(raw: string, min: number, max: number, names?: string[]): number[] {
  const all: number[] = [];
  ...
  for (const part of raw.split(",")) {
    ...
    if (stepMatch) {
      ...
      if (step < 1) throw new Error(`Invalid step value: ${stepMatch[2]}`);
    }
    ...
    if (range === "*") {
      ...
      if (lo > hi) throw new Error(`Invalid range: ${range}`);
    } else {
...
function resolveValue(s: string, min: number, max: number, names?: string[]): number {
  if (names) {
...
function describeMinute(values: number[], raw: string): string {
  ...
  if (values.length <= 4) return `at minutes ${listJoin(values.map(String))}`;
  return `at ${values.length} specific minutes`;
}

function describeHour(values: number[], raw: string): string {
  if (raw === "*") return "every hour";
  const stepMatch = raw.match(/^\*\/(\d+)$/);
  if (stepMatch) return `every ${stepMatch[1]} hours`;
  if (values.length === 1) return `at ${formatHour(values[0])}`;
  if (values.length <= 4) return `at ${listJoin(values.map(formatHour))}`;
...
function describeDom(values: number[], raw: string): string {
  if (raw === "*") return "";
  const stepMatch = raw.match(/^\*\/(\d+)$/);
  if (stepMatch) return `every ${stepMatch[1]} days`;
  if (values.length === 1) return `on the ${ordinal(values[0])}`;
  return `on days ${listJoin(values.map((v) => ordinal(v)))}`;
}

function describeMonth(values: number[], raw: string): string {
  if (raw === "*") return "";
  const stepMatch = raw.match(/^\*\/(\d+)$/);
  if (stepMatch) return `every ${stepMatch[1]} months`;
  if (values.length === 1) return `in ${MONTH_NAMES[values[0] - 1]}`;
  if (values.length <= 4) return `in ${listJoin(values.map((v) => MONTH_NAMES[v - 1]))}`;
  return `in ${values.length} months`;
}

function describeDow(values: number[], raw: string): string {
  if (raw === "*") return "";
  // Normalise 7 → 0 (both are Sunday)
  const normalised = [...new Set(values.map((v) => v % 7))].sort((a, b) => a - b);
  const stepMatch = raw.match(/^\*\/(\d+)$/);
  if (stepMatch) return `every ${stepMatch[1]} days of the week`;
  if (normalised.length === 7) return "";
  if (normalised.length === 1) return `on ${DOW_NAMES[normalised[0]]}`;
  if (normalised.length <= 4) return `on ${listJoin(normalised.map((v) => DOW_NAMES[v]))}`;
...
function buildSummary(
  minuteRaw: string, hourRaw: string, domRaw: string, monthRaw: string, dowRaw: string,
  minuteVals: number[], hourVals: number[], domVals: number[], monthVals: number[], dowVals: number[],
): string {
...
function nextRuns(
  minuteVals: number[], hourVals: number[],
  domVals: number[], monthVals: number[], dowVals: number[],
  count = 5,
): Date[] {
...
export function parseCron(raw: string): CronResult {
...
export function initCron(): void {
  ...
  const resultEl = document.getElementById("cron-result") as HTMLDivElement;
  const examples = document.querySelectorAll<HTMLButtonElement>(".cron-example");
